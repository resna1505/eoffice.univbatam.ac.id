...
class Disposisi extends CI_Controller
{
    ...
    public function hapus($id_surat = null, $id_disposisi = null)
    {
		...
		$cekPakai = $this->Disposisi_model->cekPakai($id_disposisi);
        if ($cekPakai > 0) {
            $this->session->set_flashdata('gagal', 'Data Disposisi Tidak bisa hapus, karena sudah dilihat oleh pengguna yang di disposisikan');
            #redirect(site_url('admin/surat_masuk'));
			redirect(site_url('admin/surat_masuk/tampil/' . $id_surat));
        } else {
            if ($this->Disposisi_model->delete($id_disposisi)) {
                $this->session->set_flashdata('gagal', 'Data Disposisi Berhasil di hapus');
                #redirect(site_url('admin/surat_masuk'));
				redirect(site_url('admin/surat_masuk/tampil/' . $id_surat));
            }
        }
