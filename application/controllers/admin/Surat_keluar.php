...
class Surat_keluar extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['contents'] = 'admin/v_surat_keluar';
    ...
    public function draft()
    {
        ...
        $this->data['menu'] = 'admin/v_menu';
        $this->data['contents'] = 'admin/v_draf_surat_keluar';
        $this->load->view('admin/v_home', $this->data);
    }
    ...
    public function draft_tampil($id_surat)
    {
        ...
        $this->data['menu'] = 'admin/v_menu';
        $this->data['contents'] = 'admin/v_draf_surat_keluar_tampil';
        $this->load->view('admin/v_home', $this->data);
    }
    ...
    public function draft_add()
    {
        ...
        if ($validation->run()) {
            $hasil = $data->save_draf();
            #$this->session->set_flashdata('sukses', 'Draft surat keluar sudah berhasil di tambahkan.');
            if ($hasil[0] != 'error') {
                $this->session->set_flashdata('sukses', 'Draft Surat Keluar Berhasil ditambah');
            } else {
                $this->session->set_flashdata('gagal', 'Draft Surat Keluar Gagal ditambah' . $hasil[1]);
            }
        } else {
            $this->session->set_flashdata('gagal', 'Draft surat keluar sudah di tambahkan, periksa kembali isian form yang anda masukkan.');
        }
    ...
    public function draft_edit()
    {
        ...
        if ($validation->run()) {
            $hasil = $data->update_draf();
            #$this->session->set_flashdata('sukses', 'Perubahan Draf Surat Keluar sudah berhasil di simpan');
            if ($hasil[0] != 'error') {
                $this->session->set_flashdata('sukses', 'Perubahan Draft Surat Keluar Berhasil disimpan');
            } else {
                $this->session->set_flashdata('gagal', 'Perubahan Draft Surat Keluar Gagal disimpan' . $hasil[1]);
            }
            #redirect(site_url('admin/surat_keluar/draft'));
        } else {
            $this->session->set_flashdata('gagal', 'Perubahan Draft Surat Keluar gagal di simpan');
            #redirect(site_url('admin/surat_keluar/draft'));
        }
    ...
    public function export()
    {
        ...
        foreach ($Surat as $data) { // Lakukan looping pada variabel siswa
            ...
            $sheet->setCellValue('B' . $numrow, $data->NM_KATEGORI_SRT_KELUAR);
