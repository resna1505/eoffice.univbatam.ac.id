...
class Surat_keluar extends CI_Controller
{
    ...
    public function index()
    {
        ...
        #$this->data['surat_keluar'] = $this->Surat_keluar_model->getAllDraftByApproval($this->data['user']->ID_PEGAWAI);
        $this->data['surat_keluar'] = $this->Surat_keluar_model->getAllSuratByUser($this->data['user']->ID_PEGAWAI);
        #} else {
        #    $this->data['draft_surat_keluar'] = $this->Surat_keluar_model->getAllDraft();
        #}
        #$this->data['approval'] = $this->Pegawai_login_model->getPimpinan();
        $this->data['kategori_srt_keluar'] = $this->Kategori_srt_keluar_model->getAll();
        ...
        $this->data['contents'] = 'bagian/v_surat_keluar';
    ...
    public function tampil($id_surat)
    {
        ...
        $this->data['log_surat'] = $this->Log_surat_model->getByIdSurat($id_surat);
        $this->data['surat'] = $this->Surat_keluar_model->getById($id_surat);
        $this->data['user'] = $this->Pegawai_login_model->current_user();
        $this->data['judul'] = 'Halaman untuk menampilkan surat Keluar';
        $this->data['menu'] = 'bagian/v_menu';
        $this->data['contents'] = 'bagian/v_surat_keluar_tampil';
        $this->load->view('bagian/v_home', $this->data);
    }

    public function tampilrelease($id_surat)
    {
        $this->Log_surat_keluar_model->add_log($this->Pegawai_login_model->current_user()->USERNAME, $id_surat, "Menampilkan release surat keluar");
        $this->data['log_surat'] = $this->Log_surat_keluar_model->getByIdSurat($id_surat);
        $this->data['surat'] = $this->Surat_keluar_model->getById($id_surat);
        $this->data['user'] = $this->Pegawai_login_model->current_user();
        $this->data['judul'] = 'Halaman untuk menampilkan Release Surat Keluar';
        $this->data['menu'] = 'bagian/v_menu';
        $this->data['contents'] = 'surat_keluar/v_surat_keluar_tampil_release';
        $this->load->view('bagian/v_home', $this->data);
    }
    ...
    public function draft()
    {
        ...
        #if ($this->data['user']->ID_BAGIAN != 0) {
        #$this->data['surat_keluar'] = $this->Surat_keluar_model->getAllDraftByApproval($this->data['user']->ID_PEGAWAI);
        $this->data['draft_surat_keluar'] = $this->Surat_keluar_model->getAllDraftByBagian($this->data['user']->ID_BAGIAN);
        #} else {
        #    $this->data['draft_surat_keluar'] = $this->Surat_keluar_model->getAllDraft();
        #}
        $this->data['approval'] = $this->Pegawai_login_model->getPimpinan();
        $this->data['kategori_srt_keluar'] = $this->Kategori_srt_keluar_model->getAll();
        $this->data['bagian'] = $this->Bagian_model->getAll();
        $this->data['judul'] = 'Rekapitulasi Draft Surat Keluar';
        $this->data['menu'] = 'bagian/v_menu';
        $this->data['contents'] = 'bagian/v_draf_surat_keluar';
        $this->load->view('bagian/v_home', $this->data);
    }
    ...
    public function draft_tampil($id_surat)
    {
        ...
        $this->Log_surat_keluar_model->add_log($this->Pegawai_login_model->current_user()->USERNAME, $id_surat, "Menampilkan draft surat keluar");
        $this->data['log_surat'] = $this->Log_surat_keluar_model->getByIdSurat($id_surat);
        ...
        $this->data['menu'] = 'bagian/v_menu';
        $this->data['contents'] = 'bagian/v_draf_surat_keluar_tampil';
        $this->load->view('bagian/v_home', $this->data);
    }
    ...
    public function add()
    {
        ...
        if ($validation->run()) {
            ...
            if ($hasil[0] != 'error') {
                $this->session->set_flashdata('sukses', 'Data Surat Keluar Berhasil ditambah');
            } else {
                $this->session->set_flashdata('gagal', 'Data Surat Keluar Gagal ditambah' . $hasil[1]);
            }
        } else {
            $errors = validation_errors();
            $this->session->set_flashdata('gagal', 'Validasi gagal: ' . strip_tags($errors));
            #$this->session->set_flashdata('gagal', 'Surat keluar sudah gagal di tambahkan, periksa kembali isian form yang anda masukkan.');
        }
        redirect(site_url('bagian/surat_keluar'));
    }

    public function edit()
    {
        // if (!isset($id)) redirect(site_url('bagian/surat_keluar/draft'));
        ...
        if ($validation->run()) {
            ...
            if ($hasil[0] != 'error') {
                ...
                $this->session->set_flashdata('gagal', 'Data Surat Keluar Gagal disimpan' . $hasil[1]);
            }
            #redirect(site_url('bagian/surat_keluar/draft'));
        } else {
    ...
    public function kirim_data($id)
    {
        ...
        if ($this->Surat_keluar_model->send($id)) {
            $this->session->set_flashdata('gagal', 'Surat Keluar sudah berhasil di hapus.');
            #redirect(site_url('bagian/surat_keluar/draft'));
        }
    ...
    public function release()
    {
        ...
        if ($validation->run()) {
            $hasil = $data->update();
            if ($hasil[0] != 'error') {
                $this->session->set_flashdata('sukses', 'Proses release surat keluar berhasil');
            } else {
                $this->session->set_flashdata('gagal', 'Proses release surat keluar gagal' . $hasil[1]);
            }
            $this->session->set_flashdata('sukses', 'Proses release surat keluar sudah berhasil di lakukan');
            redirect(site_url('bagian/surat_keluar'));
        } else {
            $this->session->set_flashdata('gagal', 'Proses release surat keluar gagal di simpan');
            redirect(site_url('bagian/surat_keluar'));
        }
    }
    ...
    public function draft_note()
    {
        ...
        if (!isset($id)) redirect(site_url('bagian/surat_keluar'));
        $this->Surat_keluar_model->save_note();
        redirect('bagian/surat_keluar/draft_tampil/' . $id);
    }
    ...
    public function aproved()
    {
        ...
        redirect(site_url('bagian/surat_keluar/draft_tampil/' . $id_surat_ttd)); //redirect ke halaman rekap tanda tangan surat usai simpan data
    }
    ...
    public function verify_password()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $is_valid = $this->Surat_keluar_model->verifyPassword($id_surat, $password);
