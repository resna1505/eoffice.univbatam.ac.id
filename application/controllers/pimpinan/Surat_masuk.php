...
class Surat_masuk extends CI_Controller
{
    ...
    public function index()
    {
        ...
        #}
        #$recordsurat=$this->Surat_masuk_model->getAllSuratByBagian($this->data['user']->ID_PEGAWAI);
        #echo "record=".print_r($recordsurat);exit();
        $this->data['kategori_srt_masuk'] = $this->Kategori_srt_masuk_model->getAll();
        ...
        $this->data['contents'] = 'pimpinan/v_surat_masuk';
        #print_r($this->data);
    ...
    public function tampil($id = null)
    {
        ...
        $this->data['user'] = $this->Pegawai_login_model->current_user();
        $this->data['can_approve'] = $this->Surat_masuk_model->canApprove($this->data['user']->ID_PEGAWAI);
        $this->data['surat'] = $this->Surat_masuk_model->getById($id);
        
        // Cek JENIS_SURAT_KELUAR dan PASSWORD dari t_surat_keluar
        if ($this->data['surat']->JENIS_SURAT_KELUAR == 3 && !empty($this->data['surat']->PASSWORD)) {
            $session_key = 'verified_surat_' . $id;
            if (!$this->session->userdata($session_key)) {
                $this->session->set_flashdata('gagal', 'Akses tidak diizinkan. Gunakan tombol untuk membuka surat rahasia.');
                redirect(site_url($this->uri->segment(1) . '/surat_masuk'));
                return;
            }
        }
        ...
        if ($this->data['user']->ID_BAGIAN != 0) {
            $this->data['bagian'] = $this->Pegawai_login_model->getPegawaiByBagian($this->data['user']->ID_BAGIAN);
        } else {
            $this->data['bagian'] = $this->Pegawai_login_model->getPimpinanByBagian();
        }
        ...
        $this->data['menu'] = 'pimpinan/v_menu';
        $this->data['contents'] = 'pimpinan/v_surat_masuk_tampil';
    ...
    public function verify_password()
    {
        if ($this->input->is_ajax_request() || $this->input->method() === 'post') {
            ...
            if (empty($surat_id) || empty($password)) {
                echo json_encode(['status' => 'error', 'message' => 'Data tidak lengkap']);
                return;
            }
            
            $is_valid = $this->Surat_masuk_model->verifyPassword($surat_id, $password);
            
            if ($is_valid) {
                // Set session untuk menandai bahwa password sudah diverifikasi
                $session_key = 'verified_surat_' . $surat_id;
