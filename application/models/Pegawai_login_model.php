...
class Pegawai_login_model extends CI_Model
{
    ...
    public function current_user()
    {
		...
		$this->db->select("ID_LOGIN,ID_PEGAWAI,USERNAME,PASSWORD,ID_BAGIAN,AKTIF,PIMPINAN,ID_PEGAWAI AS USER_ID");
    ...
    public function getPimpinan()
    {
		...
		$query=$this->db->get_where($this->_table,array('t_pegawai_login.PIMPINAN' => '1'));
		#echo $this->db->last_query();exit();
		return $query->result();
    }
	
	public function getPimpinanByBagian()
    {
        $this->db->select('t_pegawai_login.ID_LOGIN');
        $this->db->select('t_pegawai_login.ID_PEGAWAI');
		$this->db->select('t_pegawai_login.ID_BAGIAN');
        ...
        $this->db->join('t_pegawai', 't_pegawai.ID_PEGAWAI = t_pegawai_login.ID_PEGAWAI', 'LEFT');        
		$this->db->group_by('t_pegawai_login.ID_PEGAWAI');
		$query=$this->db->get_where($this->_table,array('t_pegawai_login.PIMPINAN' => '1','t_pegawai_login.ID_BAGIAN!=' => '0'));
		#echo $this->db->last_query();exit();
		return $query->result();
    }
	...
	public function getPegawaibyBagian($idBagian)
    {
        $this->db->select('t_pegawai_login.ID_LOGIN');
        $this->db->select('t_pegawai_login.ID_PEGAWAI');
        $this->db->select('t_pegawai_login.ID_BAGIAN');
        ...
        $this->db->join('t_pegawai', 't_pegawai.ID_PEGAWAI = t_pegawai_login.ID_PEGAWAI', 'LEFT');
        $this->db->group_by('t_pegawai_login.ID_PEGAWAI');
        return $this->db->get_where($this->_table, array('t_pegawai_login.ID_BAGIAN' => $idBagian))->result();
    }
